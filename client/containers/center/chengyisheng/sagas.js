...
// sagas
import {getMyAccount, updateAccountMobile} from '../../../sagas/data/account'
// api
import {
  getChengyishengApi,
} from '../../../apis/healthService/account'
// apis
import {sendVerificationCodeApi, checkCodeApi} from '../../../apis/healthService/account.js'
// const
...
function * watchInit() {
  while (true) {
    ...
    if(mobile) {
      yield put(getChengyisheng(urlId))
    }
    // time
...
function * watchGetChengysheng() {
  while (true) {
    ...
    try {
      ...
      yield put(getChengyishengSuccess({url}))
...
function * watchSubmit() {
  while (true) {
    const {payload: {mobile, code, urlId}} = yield take(SUBMIT)
    ...
    if (step === STEP_CHECK_OLD) { // 验证原手机号
      ...
      try {
        ...
        yield put(changeStep(STEP_UPDATE))
        yield put(getChengyisheng(urlId))
      } catch (error) {
        if (error.code === 440) {
