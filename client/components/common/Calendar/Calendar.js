/**
 * Created by caoxiaolian on 2017/8/21.
 * 日历组件
 */
...
const Calendar = React.createClass({
   ...
   */
  render() {
    require('../../../styles/common/calendar/calendar.less')
    ...
    for (let i = 0; i < month_day; i++) {
      ...
      if (tags && tags.length > 0) {
        for (let j = 0; j < tags.length; j++) {
          let dayNum = moment(tags[j].measureDate).get('date')
          let flag = false
          let weekday = moment(tags[j].measureDate).weekday();

          if(j > 0 && (moment(tags[j].measureDate).get('date') == moment(tags[j-1].measureDate).get('date')+1) && weekday != 6){
            flag = true
          }
          if ((i + 1) === dayNum) {
            if(tags[j].tag == 0){
              if(flag){
                currentClassName += ` item-long-tag`;
              }else{
              ...
              if(flag){
                currentClassName += ` item-blue-long-tag`;
              }else{
                currentClassName += ` item-blue-tag`;
    ...
    for (let i = 0; i < n_day; i++) {
      let next_link = (<li className={`item-gray li_index`+i} key={'next'+i}>
      ...
      <div className="calendar">
        ...
        <div className="calendar-body">
          ...
          <div className="c-body-content">
            {
              ul_list.map((u, index) => {
                return (<ul key={'ul'+index} className={"content-row index" + index}>{u}</ul>);
              })
            }
          </div>
        </div>
